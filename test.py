...
from tkinter import filedialog, Tk, Canvas, Button, Text
...
class ImageSelectorApp:
    def __init__(self, root):
        ...
        self.text_widget = Text(root, wrap=tk.WORD, width=30, height=5)
        self.text_widget.pack(side=tk.BOTTOM, padx=10, pady=10)
    ...
    def run_another_program(self):
        if not self.selected_portions:
            print("No selection to translate.")
            return

        # Specify the path to your Python script
        script_path = 'main-unstable.py'

        try:
            # Run the specified Python script using subprocess
            process = subprocess.run(['python', script_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            lol = process.stdout.decode()
            split = lol.split('\n')
            split = list(filter(None, split))
            print(split)

            # Display translated text on the selected portion
            for index, portion_path in enumerate(self.selected_portions):
                filename = os.path.basename(portion_path)
                coordinates = self.selection_coordinates.get(filename)

                if coordinates:
                    x1, y1, x2, y2 = coordinates
                    text_x = (x1 + x2) / 2
                    text_y = (y1 + y2) / 2  # Place text in the center of the selection box

                    # Clear any existing text on the canvas
                    self.canvas.delete(f"text_{filename}")

                    # Calculate width and height based on the size of the selected region
                    width = x2 - x1
                    height = y2 - y1

                    height_with_padding = height * 1.5

                    # Create a Text widget for each selection
                    text_widget = Text(self.canvas, wrap=tk.WORD, width=int(width / 10), height=int(height_with_padding / 20))
                    text_widget.insert(tk.END, split[index])
                    text_widget_id = self.canvas.create_window(text_x, text_y, anchor=tk.CENTER, window=text_widget, tags=f"text_{filename}")
    ...
    def on_button_release(self, event=None):
        if self.image_path and hasattr(self, 'selection_rectangle'):
            ...
            temp_image_path = os.path.join(self.temp_folder, f's_{count}.png')
